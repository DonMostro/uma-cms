...
include_once(ROOT."classes/views/VComments.php");
include_once(ROOT."classes/commands/CUsers.php");
  ...
  public function show(){
    $lang=Lang::getInstance();
  	if($this->model->getSize()>0){
		...
		//BBC Mundo
		$tpl->marca_bbc = ($this->categories_title == 'BBC Mundo') ? '
	    ...
	    if($this->searchBackgroundAds($tpl->id)){
	    	mysql_free_result();
	    	$tpl->bgad = 1;
	    	$row = $this->searchBackgroundAds($tpl->id);
	    	$tpl->bgimg = $row['image'];
	    	$tpl->bgurl = $row['url'];
	    }else{
	    	$tpl->bgad="0";
		...
		//Formulario
		$this->width=445;
		$tpl->curl="index.php?m=comments&amp;v=".$tpl->id;
   ...
   function getFeaturedVideoIdByCategory($categories_id){
  	    $mTopCarrusel = new MTopCarrusel();
    	$mTopCarrusel->setCategories_Id($categories_id);
    	$mTopCarrusel->setApproved("1");
    	$mTopCarrusel->setLimit(1);
    	$tpl->id = $mTopCarrusel->getId();
    	if(empty($tpl->id)){
    		$mVideos = new MVideos();
    		$mVideos->setCategories_Id($categories_id);
    		$mVideos->setLimit(1);
    		$tpl->id = $mVideos->getId();
    		/*if(empty($tpl->id)){
	...
	function searchBackgroundAds($videos_id){
		$qry="SELECT image, url FROM categories_background_ads 
		LEFT JOIN categories 
		ON categories_background_ads.categories_id = categories.id
		OR categories.id IN (SELECT id FROM categories WHERE parent_id = categories_background_ads.categories_id)
		LEFT JOIN videos 
		ON categories.id = videos.categories_id WHERE categories_background_ads.approved = '1' AND
		videos.id =".(int)$videos_id ." LIMIT 0, 1";
		return ($row=mysql_fetch_assoc(mysql_query($qry))) ? $row : false;
	}
}
?>