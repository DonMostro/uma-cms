...
class TestMaxUploadSize extends Test{
	...
	function run(){
		...
		if($value->getValue() > $post->getValue() || $value->getValue() > $upload->getValue()){
			...
			<p>
			  Su l&iacute;mites de subida seg&uacute;n la configuraci&oacute;n de PHP es de '.$this->_encode($limit).'. Por favor, introduzca un l&iacute;mite inferior o aumente los siguientes valores en php.ini:
			  <ul>
